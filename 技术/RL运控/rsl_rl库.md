...
现在知道奖励和回合是否结束，就能把这一步数据补齐，写入 `RolloutStorage`。

完整过程是：

**记录观测和动作 → 环境执行 → 补上奖励与 done → 写进 Storage。**
...
达到时间上限的情况通过 `infos['time_outs']` 单独标记。这份实现会先在奖励上补一项价值估计，再按回合边界截断；原笔记中的代码使用当前步保存的 `transition.values` 作为这项估计。
